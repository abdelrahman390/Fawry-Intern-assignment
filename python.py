...
class ShippableProduct(ABC):
    ...
    @abstractmethodfrom abc import ABC, abstractmethod
from typing import List
from collections import defaultdict
    ...
    for item in cart.items:
        ...
        if item.product.weight != 0:
            ...
            if(product_total_weight >= 1000):
                print(f"{item.quantity}x {item.product.name} {int(product_total_weight/1000)}kg", end=" ")
            else:
                print(f"{item.quantity}x {item.product.name} {int(product_total_weight)}g", end=" ")
        else:
            print(f"{item.quantity}x {item.product.name}", end=" ")
        print(f"{item.product.price * item.quantity}$")
